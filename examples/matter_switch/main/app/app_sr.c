...
static const sr_cmd_t g_default_cmd_info[] = {
    ...
    {SR_CMD_SWITCH_ON, SR_LANG_EN, 0, "Turn On the Switch", "TkN nN jc SWgp", {NULL}},
    {SR_CMD_SWITCH_OFF, SR_LANG_EN, 0, "Turn Off the Switch", "TkN eF jc SWgp", {NULL}},
...
esp_err_t app_sr_modify_cmd(uint32_t id, const sr_cmd_t *cmd)
{
    ...
    ESP_RETURN_ON_FALSE(NULL != it, ESP_ERR_NOT_FOUND, TAG, "can't find cmd id:%ld", cmd->id);
    return ESP_OK;
}
...
esp_err_t app_sr_remove_cmd(uint32_t id)
{
    ...
    ESP_RETURN_ON_FALSE(NULL != it, ESP_ERR_NOT_FOUND, TAG, "can't find cmd id:%ld", id);
    return ESP_OK;
}
...
const sr_cmd_t *app_sr_get_cmd_from_id(uint32_t id)
{
    ...
    ESP_RETURN_ON_FALSE(NULL != it, NULL, TAG, "can't find cmd id:%ld", id);
    return NULL;
}
